...
const ContactPage = () => {
  ...
  const fireSuccessConfetti = useCallback(() => {
    import("canvas-confetti").then(({ default: confetti }) => {
      confetti({ particleCount: 80, spread: 70, origin: { y: 0.6 }, colors: ["#6366f1", "#a855f7", "#ec4899", "#ffffff"] });
    });
  }, []);
  ...
  const copyEmail = () => {
    navigator.clipboard.writeText("tisnahidayat304@gmail.com");
    setEmailCopied(true);
    setTimeout(() => setEmailCopied(false), 2000);
    Swal.fire({ title: "Copied!", text: "Email address copied to clipboard.", icon: "success", timer: 1500, showConfirmButton: false, background: "#030014", color: "#ffffff" });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // FormSubmit endpoint
      const formSubmitUrl = 'https://formsubmit.co/tisnahidayat304@gmail.com';
  ...
  return (
    <div className="px-[5%] sm:px-[5%] lg:px-[10%] py-10 md:py-[5%]">
          ...
          >
            <div className="flex justify-between items-start mb-8">
              <div>
                ...
                <button
                  onClick={copyEmail}
                  className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 text-xs rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 hover:bg-indigo-500/20 hover:border-indigo-500/40 transition-all duration-200 group"
                >
                  <Mail className="w-3.5 h-3.5" />
                  tisnahidayat304@gmail.com
                  {emailCopied ? <Check className="w-3 h-3 text-green-400" /> : <Copy className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" />}
                </button>
              </div>
