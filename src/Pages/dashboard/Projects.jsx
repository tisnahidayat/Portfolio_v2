      ...
      <div className="p-4 flex flex-col h-full">
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-2 border-t border-white/8">
          <div className="flex gap-1.5">
            {project.github && (
              <a href={project.github !== "Private" ? project.github : undefined} target="_blank" rel="noopener noreferrer"
                className="p-1.5 rounded-lg border border-white/10 text-gray-500 hover:text-white hover:border-white/20 transition-colors" title="GitHub">
                <Github className="w-3.5 h-3.5" />
              </a>
            )}
            {project.link && (
              <a href={project.link} target="_blank" rel="noopener noreferrer"
                className="p-1.5 rounded-lg border border-white/10 text-blue-500 hover:text-blue-300 hover:border-blue-500/30 transition-colors" title="Live Demo">
                <ExternalLink className="w-3.5 h-3.5" />
              </a>
            )}
            {project.sheet_url && (
              <a href={project.sheet_url} target="_blank" rel="noopener noreferrer"
                className="p-1.5 rounded-lg border border-white/10 text-emerald-500 hover:text-emerald-300 hover:border-emerald-500/30 transition-colors" title="Test Cases Sheet">
                <FileSpreadsheet className="w-3.5 h-3.5" />
              </a>
            )}
            {project.youtube_url && (
              <a href={project.youtube_url} target="_blank" rel="noopener noreferrer"
                className="p-1.5 rounded-lg border border-white/10 text-red-500 hover:text-red-300 hover:border-red-500/30 transition-colors" title="YouTube Demo">
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        />
        <InputField
          label="Test Cases Sheet URL"
          value={form.sheet_url}
          onChange={set("sheet_url")}
...
export default function Projects() {
  ...
  const handleCreate = async (form, file) => {
    ...
    swal({ title: "Project saved!", icon: "success", timer: 1500, showConfirmButton: false });
    fetchProjects();
  };
  ...
  const handleEdit = async (form, file) => {
    ...
    setUploading(false);
    if (error) { swal({ title: "Error", text: error.message, icon: "error" }); return; }
    swal({ title: "Project updated!", icon: "success", timer: 1500, showConfirmButton: false });
    fetchProjects();
  };
