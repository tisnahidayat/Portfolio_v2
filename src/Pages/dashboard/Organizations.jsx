...
import { Users, Plus, Trash2, Pencil, Upload, X } from "lucide-react";
...
const OrgForm = ({ initial, onSubmit, onCancel, submitLabel = "Save", uploading }) => {
    ...
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form, file); }} className="p-5 sm:p-6 space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2">
          <InputField label="Organization Name *" value={form.name} onChange={set("name")} placeholder="e.g. Himpunan Mahasiswa Informatika" required />
        </div>
        <InputField label="Role / Position" value={form.role} onChange={set("role")} placeholder="e.g. Head of Technology" />
        <InputField label="Start Date" value={form.start_date} onChange={set("start_date")} placeholder="e.g. 2021" />
        <InputField label="End Date" value={form.end_date} onChange={set("end_date")} placeholder="e.g. 2022" />
        ...
        <div className="sm:col-span-2 space-y-1.5">
          <label className="text-xs text-indigo-300/70 uppercase tracking-wider font-medium">Organization Logo</label>
...
const OrgCard = ({ item, onEdit, onDelete }) => (
  <Card>
    <div className="p-4 flex items-start gap-4">
      <div className={`w-12 h-12 rounded-xl overflow-hidden flex items-center justify-center shrink-0 ${item.img ? "" : "border border-white/10 bg-white/5"}`}>
        {item.img ? <img src={item.img} alt={item.name} className="w-full h-full object-cover" /> : <Users className="w-5 h-5 text-pink-400" />}
      </div>
      <div className="flex-1 min-w-0">
        <h3 className="font-semibold text-white text-sm truncate mb-1">{item.name}</h3>
        <p className="text-pink-400 text-xs mb-1">{item.role}</p>
...
export default function Organizations() {
  ...
  const fetch = async () => {
    setLoading(true);
    const { data } = await supabase.from("organizations").select("*").order("sort_order", { ascending: true });
    setItems(data || []);
    setLoading(false);
  };
  ...
  const handleCreate = async (form, file) => {
    ...
    swal({ title: "Organization saved!", icon: "success", timer: 1500, showConfirmButton: false });
    fetch();
  };
  ...
  const handleEdit = async (form, file) => {
    ...
    setUploading(false);
    if (error) { swal({ title: "Error", text: error.message, icon: "error" }); return; }
    swal({ title: "Organization updated!", icon: "success", timer: 1500, showConfirmButton: false });
    fetch();
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-white">Organizations</h1>
        ...
        <button onClick={() => setShowCreate(true)} className="relative group shrink-0">
          ...
          <div className="relative flex items-center gap-2 px-4 py-2.5 bg-[#030014] rounded-xl border border-white/10">
            ...
            <span className="text-sm text-gray-200">Add Organization</span>
          </div>
        </button>
      </div>

      {showCreate && (
        <Modal title="Add Organization" onClose={() => setShowCreate(false)}>
          <OrgForm onSubmit={handleCreate} onCancel={() => setShowCreate(false)} submitLabel="Save Organization" uploading={uploading} />
        </Modal>
      )}
      {editItem && (
        <Modal title="Edit Organization" onClose={() => setEditItem(null)}>
          <OrgForm initial={editItem} onSubmit={handleEdit} onCancel={() => setEditItem(null)} submitLabel="Update Organization" uploading={uploading} />
        </Modal>
      )}
      ...
      {loading ? (
        ...
        <Card><div className="p-16 text-center"><Users className="w-10 h-10 text-gray-700 mx-auto mb-3" /><p className="text-gray-500 text-sm">No organizations yet.</p></div></Card>
      ) : (
        <div className="space-y-3">{items.map((item) => <OrgCard key={item.id} item={item} onEdit={setEditItem} onDelete={handleDelete} />)}</div>
      )}
    </div>
  );
}
