...
import { GraduationCap, Plus, Trash2, Pencil, Upload, X } from "lucide-react";
...
const EducationForm = ({ initial, onSubmit, onCancel, submitLabel = "Save", uploading }) => {
    ...
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form, file); }} className="p-5 sm:p-6 space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2">
          <InputField label="Institution *" value={form.institution} onChange={set("institution")} placeholder="e.g. Universitas Indonesia" required />
        </div>
        <InputField label="Degree" value={form.degree} onChange={set("degree")} placeholder="e.g. Bachelor of Computer Science" />
        <InputField label="Field of Study" value={form.field} onChange={set("field")} placeholder="e.g. Informatics Engineering" />
        <InputField label="Start Year" value={form.start_date} onChange={set("start_date")} placeholder="e.g. 2019" />
        <InputField label="End Year" value={form.end_date} onChange={set("end_date")} placeholder="e.g. 2023" />

        <div className="sm:col-span-2 space-y-1.5">
          <label className="text-xs text-indigo-300/70 uppercase tracking-wider font-medium">Description</label>
          <textarea value={form.description} onChange={set("description")} rows={3}
            placeholder="Describe your studies, thesis, or notable achievements..."
        ...
        <div className="sm:col-span-2 space-y-1.5">
          <label className="text-xs text-indigo-300/70 uppercase tracking-wider font-medium">Institution Logo</label>
...
const EducationCard = ({ item, onEdit, onDelete }) => (
  <Card>
    <div className="p-4 flex items-start gap-4">
      <div className={`w-12 h-12 rounded-xl overflow-hidden flex items-center justify-center shrink-0 ${item.img ? "" : "border border-white/10 bg-white/5"}`}>
        {item.img ? <img src={item.img} alt={item.institution} className="w-full h-full object-cover" /> : <GraduationCap className="w-5 h-5 text-purple-400" />}
      </div>
      <div className="flex-1 min-w-0">
        <h3 className="font-semibold text-white text-sm truncate mb-1">{item.institution}</h3>
        <p className="text-purple-400 text-xs mb-1">{item.degree}{item.field ? ` — ${item.field}` : ""}</p>
        <p className="text-gray-500 text-xs">{item.start_date} — {item.end_date}</p>
...
export default function Education() {
  ...
  const handleCreate = async (form, file) => {
    ...
    const { error } = await supabase.from("education").insert({
      ...
      description: form.description, img: imgUrl || null,
      sort_order: Number(form.sort_order) || 0,
    });
    setShowCreate(false);
    setUploading(false);
    if (error) { swal({ title: "Error", text: error.message, icon: "error" }); return; }
    swal({ title: "Education saved!", icon: "success", timer: 1500, showConfirmButton: false });
    fetch();
  };
  ...
  const handleEdit = async (form, file) => {
    ...
    setUploading(false);
    if (error) { swal({ title: "Error", text: error.message, icon: "error" }); return; }
    swal({ title: "Education updated!", icon: "success", timer: 1500, showConfirmButton: false });
    fetch();
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-white">Education</h1>
        ...
        <button onClick={() => setShowCreate(true)} className="relative group shrink-0">
          ...
          <div className="relative flex items-center gap-2 px-4 py-2.5 bg-[#030014] rounded-xl border border-white/10">
            ...
            <span className="text-sm text-gray-200">Add Education</span>
          </div>
        </button>
      </div>

      {showCreate && (
        <Modal title="Add Education" onClose={() => setShowCreate(false)}>
          <EducationForm onSubmit={handleCreate} onCancel={() => setShowCreate(false)} submitLabel="Save Education" uploading={uploading} />
        </Modal>
      )}
      {editItem && (
        <Modal title="Edit Education" onClose={() => setEditItem(null)}>
          <EducationForm initial={editItem} onSubmit={handleEdit} onCancel={() => setEditItem(null)} submitLabel="Update Education" uploading={uploading} />
        </Modal>
      )}

      {loading ? (
        <div className="space-y-3">{Array.from({ length: 2 }).map((_, i) => <div key={i} className="h-24 bg-white/5 animate-pulse rounded-2xl border border-white/10" />)}</div>
      ) : items.length === 0 ? (
        <Card><div className="p-16 text-center"><GraduationCap className="w-10 h-10 text-gray-700 mx-auto mb-3" /><p className="text-gray-500 text-sm">No education yet.</p></div></Card>
      ) : (
        <div className="space-y-3">{items.map((item) => <EducationCard key={item.id} item={item} onEdit={setEditItem} onDelete={handleDelete} />)}</div>
      )}
    </div>
  );
}
